...
const AUTO_SCAN_CONFIG = {
  ENABLED: true,
  // Sectors to scan automatically (smaller sectors for faster results)
...
async function screenSector(sectorName, chatId, progressCallback) {
  ...
  for (const symbol of stocks) {
    ...
    if (progressCallback && (processed % 5 === 0 || processed === total)) {
...
async function performAutoScan(sessionName) {
  ...
  for (const sectorName of AUTO_SCAN_CONFIG.SECTORS_TO_SCAN) {
    try {
      const results = await screenSector(sectorName, null, null);
      const withSignals = results.filter(r => r.signal && !r.error);
      
      if (withSignals.length > 0) {
        allResults.push({ sector: sectorName, results: withSignals });
      }
    } catch (error) {
  ...
  console.log(`Auto-scan completed. Notified ${subscribers.size} subscribers.`);
}
